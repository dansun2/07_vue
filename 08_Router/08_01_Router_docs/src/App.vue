...
<!-- 
  routing
  서버 라우팅은 클라이언트에서 요청한 url 리소스에 따라 서버에서 어떤 작업을 수행할 지 결정하는 것을 의미한다.
  서버 라우팅 방식은 새로운 페이지 요청을 서버로 전달하면 서버측에서 html을 응답하고
  응답받은 html을 브라우저에 다시 로드하는 과정이다.

  그러나 spa(single-page-application)에서는 클라이언트 측 자바스크립트가 전체 페이지를 다시 로드하지 않고
  일부분만 동적으로 갱신되는 애플리케이션을 의미하며 서버 라우팅은 웹 애플리케이션에 새로운 페이지를 로드할 때 마다
  ...
  vue에서 라우팅을 하기 위해서 vue router를 설치해야 한다.
  npm install vue-router@next

  vue의 routing
  vue는 url 리소스를 설정할 때 url 속성에 #을 추가하여 리소스를 등록하게 되는데
  이는 vue에서 브라우저의 기본 동작을 방지하기 위함으로 만약 #를 추가하지 않으면
